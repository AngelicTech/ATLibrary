...
uses
  ...
  AT.Vcl.Actions.Data, AT.ShortName.Types, cxPC, Vcl.Forms;
...
type
  ...
  strict protected
    ...
    procedure _UpdateIntfActionAC<TCapIntf, TQryIntf>(Sender: TObject;
        Target: TCustomForm; AVisibleIID: TGUID; AVisibleFunc:
...
uses
  AT.GarbageCollector, AT.ShortName.Vcl.Dialogs.SplashDX, Vcl.Menus,
...
var
  AIntf: IATClipboard;
begin
  if (AppServices.ActiveChildSupports(IATClipboard,
      AIntf)) then
...
var
  ADocIntf: IATDocFile;
begin
  if (AppServices.ActiveChildSupports(IATDocFile,
      ADocIntf)) then
...
var
  ...
  ADocIntf: IATDocFile;
begin
  if ( AppServices.HasActiveChild ) then
    begin
      Cnt := AppServices.MainForm.MDIChildCount;

      for Idx := 0 to (Cnt - 1) do
        begin
          AFrm := AppServices.MainForm.MDIChildren[Idx];

          AOk := (Supports(AFrm, IATDocFile, ADocIntf));
...
var
  AIntf: IATInsOvr;
begin
  if (NOT AppServices.ActiveChildSupports(IATInsOvr,
      AIntf)) then
    Exit;
...
var
  AIntf: IATSearch;
begin
  if (AppServices.ActiveChildSupports(IATSearch,
      AIntf)) then
    AIntf.Find;
end;
...
var
  AIntf: IATSelection;
begin
  if (AppServices.ActiveChildSupports(IATSelection,
      AIntf)) then
    AIntf.SelectAll;
end;
...
begin
  ...
  if (AVisible) then
    AVisible := (AppServices.ActiveChildSupports(
        IATDocFile, ADocIntf));
  ...
  if (AEnable) then
    begin
      AEnable := (AppServices.ActiveChildSupports(
    ...
    begin
      ...
      for Idx := 0 to (Cnt - 1) do
        begin
          ...
          AVisible := (Supports(AFrm, IATDocFile, ADocIntf));

          if (AVisible) then
            begin
              AEnable := (Supports(AFrm, IATDocFileQuery, AQryIntf));
...
procedure TdmActions._UpdateIntfActionAC<TCapIntf, TQryIntf>(Sender:
    TObject; Target: TCustomForm; AVisibleIID: TGUID; AVisibleFunc:
    TATUpdateIntfFunc<TCapIntf>; AEnableIID: TGUID; AEnableFunc:
    TATUpdateIntfFunc<TQryIntf>);
var
  bVisible: Boolean;
