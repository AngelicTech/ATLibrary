...
uses
  ...
  AT.Vcl.Actions.Data, AT.ShortName.Types, cxPC;
...
type
  ...
  TdmActions = class(TDataModule)
    ...
    actFileNewBlank: TATBooleanDataAction;
    actFileOpen: TATBooleanDataAction;
    actFileSave: TATBooleanDataAction;
    actFileSaveAll: TATBooleanDataAction;
    actFileSaveAs: TATBooleanDataAction;
    ...
    actQATFileNewBlank: TATBooleanDataAction;
    actQATFileOpen: TATBooleanDataAction;
    actQATFileSave: TATBooleanDataAction;
    actQATFileSaveAll: TATBooleanDataAction;
    actQATFileSaveAs: TATBooleanDataAction;
    ...
    procedure ExecuteFileNewBlankAction(Sender: TObject);
    procedure ExecuteFileOpenAction(Sender: TObject);
    procedure ExecuteFileSaveAction(Sender: TObject);
    procedure ExecuteFileSaveAllAction(Sender: TObject);
    procedure ExecuteFileSaveAsAction(Sender: TObject);
    ...
    procedure UpdateFileSaveAction(Sender: TObject);
    procedure UpdateFileSaveAllAction(Sender: TObject);
    procedure UpdateFileSaveAsAction(Sender: TObject);
  ...
  strict protected
    ...
    procedure _UpdateIntfActionAC<TCapIntf, TQryIntf>(Sender: TObject;
        Target: TcxTabSheet; AVisibleIID: TGUID; AVisibleFunc:
        TATUpdateIntfFunc<TCapIntf>; AEnableIID: TGUID; AEnableFunc:
        TATUpdateIntfFunc<TQryIntf>);
    /// <summary>
  ...
  public
    ...
    class procedure SetActionShortcut(AAction: TBasicAction; const
        AShortcut: String); overload;
    class procedure SetActionShortcut(AAction: TBasicAction; const
...
uses
  ...
  AT.ShortName.ResourceStrings, System.Rtti, Vcl.Menus,
  ...
  AT.Windows, AT.ShortName.Consts;
...
begin
  ...
  TdlgSplashDX.ChangeStartMessage(rstrStartInitActions,
      cStartMsgDelay);
...
var
  AIntf: IATClipboard;
begin
  if (AppServices.ActiveChildFormOrFrameSupports(IATClipboard,
      AIntf)) then
...
var
  AIntf: IATUndo;
begin
  if (AppServices.ActiveChildFormOrFrameSupports(IATUndo, AIntf)) then
    AIntf.Redo;
end;
...
procedure TdmActions.ExecuteFileNewBlankAction(Sender: TObject);
begin
//  TfrmChildDocument.CreateNewDocument;
end;

procedure TdmActions.ExecuteFileOpenAction(Sender: TObject);
begin
//  TfrmChildDocument.CreateOpenDocument;
end;

procedure TdmActions.ExecuteFileSaveAction(Sender: TObject);
var
  ADocIntf: IATDocFile;
begin
  if (AppServices.ActiveChildFormOrFrameSupports(IATDocFile,
      ADocIntf)) then
    ADocIntf.SaveDocument;
end;

procedure TdmActions.ExecuteFileSaveAllAction(Sender: TObject);
var
  AMain: IATMainTabUI;
  Cnt: Integer;
  Idx: Integer;
  ...
  ADocIntf: IATDocFile;
begin
  if ( (AppServices.MainFormSupports(IATMainTabUI, AMain)) AND
       AppServices.HasActiveChild ) then
    begin
      Cnt := AMain.TabChildCount;

      for Idx := 0 to (Cnt - 1) do
        begin
          ATab := AMain.TabChildren[Idx];

          AOk := (AppServices.ChildFormOrFrameSupports(ATab,
              IATDocFile, ADocIntf));
...
procedure TdmActions.ExecuteFileSaveAsAction(Sender: TObject);
var
  ADocIntf: IATDocFile;
begin
  if (AppServices.ActiveChildFormOrFrameSupports(IATDocFile,
      ADocIntf)) then
    ADocIntf.SaveDocumentAs;
end;
...
var
  AIntf: IATInsOvr;
begin
  if (NOT AppServices.ActiveChildFormOrFrameSupports(IATInsOvr,
      AIntf)) then
    Exit;
...
var
  AIntf: IATSearch;
begin
  if (AppServices.ActiveChildFormOrFrameSupports(IATSearch,
      AIntf)) then
    AIntf.Find;
end;
...
var
  AIntf: IATSelection;
begin
  if (AppServices.ActiveChildFormOrFrameSupports(IATSelection,
      AIntf)) then
    AIntf.SelectAll;
end;
...
class procedure TdmActions.SetActionShortcut(AAction: TBasicAction;
...
begin
  ...
  if (AShortcut.IsEmpty) then
    begin
      ObjectServices.SetObjectPropValue(AAction, 'Shortcut', NIL);
    end
  else
    begin
      ASC := TextToShortCut(AShortcut);
      TdmActions.SetActionShortcut(AAction, ASC);
    end;
end;
...
begin
  ...
  ObjectServices.SetObjectPropValue(AAction, 'Shortcut', AShortcut);
end;
...
begin
  _UpdateIntfActionAC<IATClipboardCaps, IATClipboardQuery>(Sender,
...
begin
  _UpdateIntfActionAC<IATUndoCaps, IATUndoQuery>(Sender,
      AppServices.ActiveChild,
      IATUndoCaps,
...
procedure TdmActions.UpdateFileSaveAction(Sender: TObject);
var
  AVisible: Boolean;
  ADocIntf: IATDocFile;
  AEnable: Boolean;
  AQryIntf: IATDocFileQuery;
begin
  AVisible := AppServices.HasActiveChild;

  if (AVisible) then
    AVisible := (AppServices.ActiveChildFormOrFrameSupports(
        IATDocFile, ADocIntf));

  ObjectServices.SetObjectVisible(Sender, AVisible);

  AEnable := AVisible;

  if (AEnable) then
    begin
      AEnable := (AppServices.ActiveChildFormOrFrameSupports(
          IATDocFileQuery, AQryIntf));

      if (AEnable) then
        AEnable := AQryIntf.CanSaveDocument;
    end;

  ObjectServices.SetObjectEnabled(Sender, AEnable);
end;

procedure TdmActions.UpdateFileSaveAllAction(Sender: TObject);
var
  bVisible: Boolean;
  bEnable: Boolean;
  AVisible: Boolean;
...
begin
  ...
  AVisible := ( (AppServices.MainFormSupports(IATMainTabUI, AMain))
                AND AppServices.HasActiveChild );
    ...
    begin
      ...
      for Idx := 0 to (Cnt - 1) do
        begin
          ...
          AVisible := (AppServices.ChildFormOrFrameSupports(ATab,
              IATDocFile, ADocIntf));

          if (AVisible) then
            begin
                AEnable := (AppServices.ChildFormOrFrameSupports(ATab,
                    IATDocFileQuery, AQryIntf));

              if (AEnable) then
                AEnable := AQryIntf.CanSaveDocument;

              bEnable := (bEnable OR AEnable);
            end;

          bVisible := (bVisible OR AVisible);
        end;
    end;
...
procedure TdmActions.UpdateFileSaveAsAction(Sender: TObject);
var
  AVisible: Boolean;
  ADocIntf: IATDocFile;
  AEnable: Boolean;
...
begin
  ...
  if (AEnable) then
    begin
      ...
      if (AEnable) then
        AEnable := AQryIntf.CanSaveDocumentAs;
    end;
...
begin
  _UpdateIntfActionAC<IATSearchCaps, IATSearchQuery>(Sender,
      AppServices.ActiveChild,
      IATSearchCaps,
...
begin
  _UpdateIntfActionAC<IATSelectionCaps, IATSelectionQuery>(Sender,
      AppServices.ActiveChild,
      IATSelectionCaps,
...
var
  AFrm: IATMainUI;
  AFraChild: IATTabFrameUI;
  AFrmChild: IATTabFormUI;
begin
  //Tell main form to synchronize its UI.
  if (AppServices.MainFormSupports(IATMainUI, AFrm)) then
    AFrm.SynchronizeUIObjects;

  //Tell active child frame to synchronize its UI.
  if (AppServices.ActiveChildFrameSupports(IATTabFrameUI,
      AFraChild)) then
    AFraChild.SynchronizeUIObjects;

  //Tell active child form to synchronize its UI.
  if (AppServices.ActiveChildFormSupports(IATTabFormUI,
      AFrmChild)) then
    AFrmChild.SynchronizeUIObjects;
end;
...
procedure TdmActions._UpdateIntfActionAC<TCapIntf, TQryIntf>(Sender:
    TObject; Target: TcxTabSheet; AVisibleIID: TGUID; AVisibleFunc:
    TATUpdateIntfFunc<TCapIntf>; AEnableIID: TGUID; AEnableFunc:
    TATUpdateIntfFunc<TQryIntf>);
var
  bVisible: Boolean;
...
begin
  ...
  if (bVisible) then
    begin
      bVisible := (AppServices.ChildFormOrFrameSupports(Target,
          AVisibleIID, ACaps));
  ...
  bEnable := (AppServices.ChildFormOrFrameSupports(Target,
      AEnableIID, AQry));
