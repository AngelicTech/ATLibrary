...

{
  ...
  By default, the application's own preferences file is read and written to.
  This can be changed however by passing an appropriate Mac bundle identifier
  to the constructor, e.g. IniFile := TMacPreferencesIniFile.Create(
     'com.apple.Safari'). The preferences API itself works in terms of simple
     Name=Value pairs. While a value could itself be a name=value list, the
     dictionary object returned by the API in such a case is immutable. This
     means changing just a single nested value requires the whole section to be
     read, copied, amended, and rewritten. Yucky-yuck!
...
type
  ...
  public
    ...
    function ReadBinaryStream(const Section, Ident: string; Stream: TStream):
      Integer; override;
    ...
    procedure ReadSections(const ParentSection: string; Strings: TStrings);
      override;
    ...
    function ReadFloat(const Section, Ident: string; Default: Double): Double;
      override;
    function ReadDouble(const Section, Ident: string; Default: Double): Double;
    ...
    procedure WriteBinaryStream(const Section: string; const Ident: string;
    ...
    procedure WriteDate(const Section, Ident: string; Value: TDateTime);
      override;
    procedure WriteInteger(const Section: string; const Ident: string; Value:
       Int32); override;
    procedure WriteFloat(const Section: string; const Ident: string; Value:
       Double); override;
    procedure WriteDouble(const Section: string; const Ident: string; Value:
       Double);
...
begin
  ...
  Result := EncodeDateTime(GregRec.year, GregRec.month, GregRec.day,
    GregRec.hour, GregRec.minute, Trunc(GregRec.second), Trunc(Frac(
       GregRec.second) * 1000));
end;
...
begin
  ...
  begin
    ...
    if BundleHandle <> nil then
      FApplicationIDStr := CFStringGetValue(CFBundleGetIdentifier(
         BundleHandle));
...
begin
  ...
  begin
    Keys := CFPreferencesCopyKeyList(FApplicationID, kCFPreferencesCurrentUser,
...
function TApplePreferencesIniFile.ReadBoolean(const Section, Ident: string;
    Default: Boolean): Boolean;
begin
...
function TApplePreferencesIniFile.ReadDouble(const Section, Ident: string;
  Default: Double): Double;
begin
...
procedure TApplePreferencesIniFile.ReadSection(const Section: string; Strings:
   TStrings);
var
...
procedure TApplePreferencesIniFile.ReadSections(const ParentSection: string;
   Strings: TStrings);
begin
...
procedure TApplePreferencesIniFile.ReadSectionValues(const Section: string;
   Strings: TStrings);
var
  Ident: string;
  I: Integer;
begin
  ...
  try
    ...
    begin
      Ident := Strings[I];
      Strings[I] := Ident + Strings.NameValueSeparator + ReadString(Section,
         Ident, '');
    end;
  finally
    Strings.EndUpdate;
...
function TApplePreferencesIniFile.ReadString(const Section, Ident, Default:
   string): string;
var
  TypeID: CFTypeID;
  Value: CFPropertyListRef;
...
function TApplePreferencesIniFile.ReadValue(const Section, Ident: string):
  CFPropertyListRef;
var
  Key: CFStringRef;
begin
...
procedure TApplePreferencesIniFile.WriteBinaryStream(const Section, Ident:
...
procedure TApplePreferencesIniFile.WriteBool(const Section, Ident: string;
    Value: Boolean);
begin
  if Value then
...
procedure TApplePreferencesIniFile.WriteBoolean(const Section, Ident: string;
    Value: Boolean);
begin
  WriteBool(Section, Ident, Value);
end;

procedure TApplePreferencesIniFile.WriteDateTime(const Section, Ident: string;
   Value: TDateTime);
begin
  WriteValue(Section, Ident, CFDateCreateFromTDateTime(Value), True);
end;

procedure TApplePreferencesIniFile.WriteDate(const Section, Ident: string;
   Value: TDateTime);
begin
  WriteDateTime(Section, Ident, Value);
end;

procedure TApplePreferencesIniFile.WriteFloat(const Section, Ident: string;
   Value: Double);
begin
  WriteValue(Section, Ident, CFNumberCreate(nil, kCFNumberFloat64Type, @Value),
     True);
end;

procedure TApplePreferencesIniFile.WriteDouble(const Section, Ident: string;
   Value: Double);
begin
  WriteFloat(Section, Ident, Value);
end;

procedure TApplePreferencesIniFile.WriteInteger(const Section, Ident: string;
   Value: Int32);
begin
  WriteValue(Section, Ident, CFNumberCreate(nil, kCFNumberSInt32Type, @Value),
     True);
end;

procedure TApplePreferencesIniFile.WriteString(const Section, Ident, Value:
   string);
begin
...
begin
  FAllKeysCache := nil;
  S := CFStringCreate(Key);
  //*not* CFStringCreateNoCopy since CFPreferencesSetAppValue retains it
  try
...
function TApplePreferencesIniFile.ValueExists(const Section, Ident: string):
  Boolean;
var
  Key, ToFind: string;
begin
