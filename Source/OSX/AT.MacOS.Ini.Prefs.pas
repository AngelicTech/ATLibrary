...
// Framework      : None
//
// Filename       : AT.MacOS.Ini.Prefs.pas
...
// Description:
//
// Implements a TCustomIniFile descendant that delegates to the Core
...
/// <summary>
///   Implements a TCustomIniFile descendant that delegates to the
///   Core Foundation preferences API.
/// </summary>
/// <remarks>
///   <para>
///     Types native to that API are written, so that WriteBool
///     writes a CFBool, WriteInteger and WriteFloat a
///     CFNumber, WriteDate and WriteDateTime a CFDate, and
///     WriteBinaryStream a CFData object. Also, UpdateFile is
///     mapped to CFPreferencesAppSynchronize, and the
///     inherited FileName property returns the path to
///     relevant .plist file under ~/Library/Preferences.
///   </para>
///   <para>
///     By default, the application's own preferences file is read
///     and written to. This can be changed however by
///     passing an appropriate Mac bundle identifier to the
///     constructor, e.g. IniFile :=
///     TMacPreferencesIniFile.Create('com.apple.Safari').
///   </para>
///   <para>
///     The preferences API itself works in terms of simple
///     Name=Value pairs. While a value could itself be a
///     name=value list, the dictionary object returned by the
///     API in such a case is immutable. This means changing just a
///     single nested value requires the whole section to be
///     read, copied, amended, and rewritten. Yucky-yuck!
///   </para>
///   <para>
///     Because of this, I've mapped the INI file structure to the
///     top level name/ value list only, using a delimeter (a
///     colon by default) to separate section name from key
///     name. This is what TextWrangler does, for example, along
///     with Microsoft Word (well, Word uses a backslash).
///     TCustomIniFile's admittedly somewhat hacked-on
///     sub-section interface is also supported using this scheme.
///   </para>
///   <para>
///     One word of warning: since the underlying API is case
///     sensitive, so is this wrapper.
///   </para>
/// </remarks>
unit AT.MacOS.Ini.Prefs;
